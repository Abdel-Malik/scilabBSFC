...
function res = ff(x,y,A)
    res = (A(1).*(x.^2))+(A(2).*(y.^2))+(A(3).*(x))+(A(4).*(y))+(A(5).*(y.*x))+A(6)+5.9;
endfunction
...
function res = matriceVal3D(x,mcP,mcC)
    ...
    c=c';
    res = [ (x.^2)' (p.^2) x' p (p.*x') ones(size(x,2),1)];
...
function res = afficheConsoPC(x,y,Z,X)
    ...
    for i = (1:3:p)
        a = fMC(x(i),X);
            q = 0;
        for j = (1:2:p)
            if((Z(i,j) >= a-0.5) & (Z(i,j) <= a+0.5) & q < 50) then
                plot(x(i),y(j),'x');
                res = res + 1;
                q = q+1;
                j = j+1;
...
a = linspace(700,2500,400);

[A,B] = meshgrid(x,y);
Z = ff(A,B,matriceVal3D(a,X2,X));
Z = Z';
//Z = gg2(Z,rpm,X,1.2)
f=gcf();f.color_map=hotcolormap(22);xtitle("interpolation BSFC : f(x,y) =a*x^2 + b*x + c*y^2 + d*y + e*y*x + f+5.9","regime moteur (tr/min)","puissance fourni (kW)")
